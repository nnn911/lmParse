...
class LogBase:
    ...
    def dowloadRemote(self, fname):
        ...
        localFile = os.path.join(tempfile.gettempdir(), str(abs(hash(remoteFile))))
...
class Log(LogBase):
    ...
    def converged(self, arg):
        ...
        converged = ["force tolerance", "energy tolerance", "linesearch alpha is zero"]
        try:
    ...
    def parseLog(self, fname):
        ...
        with open(fname, "r") as f:
            for line in f:
                if (line[:12] == "Memory usage") or (line[:12] == "Per MPI rank"):
                    headerline = True
                    tailline = False
                    continue
                if line[:4] == "Loop":
...
class elasticConstantsLog:
        ...
        with open(fname, "r") as f:
            for line in f:
                ...
                if match:
                    ...
                    self.data[match.groups()[0].lower()] = float(match.groups()[1])
        if len(self.data) == 0:
    ...
    def _getVRH(self):
        if np.isclose(np.sum(self.Smat), 0) or np.isclose(np.sum(self.Cmat), 0):
            self._createCmat()
        self.data["kv"] = (
            (self.Cmat[0, 0] + self.Cmat[1, 1] + self.Cmat[2, 2])
            + 2 * (self.Cmat[0, 1] + self.Cmat[1, 2] + self.Cmat[2, 0])
        ) / 9
        self.data["gv"] = (
            (self.Cmat[0, 0] + self.Cmat[1, 1] + self.Cmat[2, 2])
            - (self.Cmat[0, 1] + self.Cmat[1, 2] + self.Cmat[2, 0])
            + 3 * (self.Cmat[3, 3] + self.Cmat[4, 4] + self.Cmat[5, 5])
        ) / 15
        self.data["kr"] = 1 / (
            (self.Smat[0, 0] + self.Smat[1, 1] + self.Smat[2, 2])
            + 2 * (self.Smat[0, 1] + self.Smat[1, 2] + self.Smat[2, 0])
        )
        self.data["gr"] = 15 / (
            4 * (self.Smat[0, 0] + self.Smat[1, 1] + self.Smat[2, 2])
        ...
        self.data["nu"] = 0.5 * (
            1 - (3 * self.data["g"]) / (3 * self.data["k"] + self.data["g"])
        )
